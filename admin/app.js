...
const permissionByModule = {
  ...
  orders: "operations.manage",
  delivery_jobs: "operations.manage",
  support: "operations.manage",
  documents: "documents.review",
  enforcements: "accounts.enforce",
  states: "registrations.manage",
  fairs: "registrations.manage",
  stalls: "registrations.manage",
  users: "registrations.manage",
  vendors: "registrations.manage",
  drivers: "registrations.manage",
  products: "registrations.manage",
  categories: "registrations.manage",
  regions: "registrations.manage",
  vehicle_rules: "rules.manage",
  delivery_fees: "rules.manage",
  platform_fees: "rules.manage",
  payment_methods: "rules.manage",
  cancellation_reasons: "rules.manage",
  onboarding_requirements: "rules.manage",
  finance: "finance.manage",
  payments: "finance.manage",
  promotions: "finance.manage",
  payouts: "finance.manage",
  reviews: "finance.manage",
  content: "communications.manage",
  announcements: "communications.manage",
  notifications: "communications.manage",
  settings: "settings.manage",
  features: "settings.manage",
  integrations: "settings.manage",
  integration_health: "settings.manage",
...
function show(id) {
  ["boot", "setupView", "loginView", "mfaView", "appView"].forEach((x) =>
...
function title(v) {
  return String(v).replace(/(^|\s)\S/g, (m) => m.toUpperCase());
}
function money(v) {
  const n = Number(v);
...
const labels = {
  ...
  city: "Cidade",
  state: "UF",
  receiving_method: "Recebimento",
  profile_id: "Usuário",
  document_type: "Documento",
  status: "Status",
  expires_at: "Validade",
  correction_reason: "Correção",
  price: "Preço",
  promotion_price: "Promo",
  stock: "Estoque",
  unit: "Unidade",
  available: "Disponível",
  default_radius_km: "Raio",
  customer_orders_enabled: "Pedidos",
  vendor_registration_enabled: "Feirantes",
  delivery_enabled: "Entregas",
  display_name: "Veículo",
  default_capacity_kg: "Capacidade kg",
  requires_plate: "Placa",
  requires_vehicle_document: "Documento",
  requires_cnh: "CNH",
  active: "Ativo",
  priority: "Prioridade",
  base_fee: "Base",
  ...
  minimum_driver_payout: "Mín. entregador",
  scope: "Escopo",
  percentage: "%",
  fixed_amount: "Fixo",
  label: "Nome",
  method_type: "Tipo",
  customer_enabled: "Cliente",
  vendor_enabled: "Feirante",
  requires_online_provider: "Provedor",
  actor_role: "Papel",
  flow_stage: "Etapa",
  requires_details: "Detalhes",
  opens_support_ticket: "Suporte",
  penalty_amount: "Penalidade",
  profile_role: "Papel",
  vehicle_type: "Veículo",
  required: "Obrigatório",
  critical: "Crítico",
  expiration_required: "Validade",
  payment_status: "Pagamento",
  subtotal: "Subtotal",
  delivery_fee: "Frete",
  total: "Total",
  order_id: "Pedido",
  delivery_id: "Entregador",
  ...
  vendor_pays_delivery: "Patrocina frete",
  starts_at: "Início",
  ends_at: "Fim",
  amount: "Valor",
  provider_reference: "Provedor",
  ...
  stall_code: "Box",
  stall_name: "Banca / box",
  fair_id: "Feira",
  vendor_id: "Feirante",
  method: "Método",
  provider_fee: "Taxa provedor",
  platform_amount: "Feiraê",
  vendor_amount: "Feirante",
  delivery_amount: "Entregador",
...
function label(k) {
  return labels[k] || title(k.replaceAll("_", " "));
}
...
const reportDefs = {
  orders: {
    ...
    cols: ["id", "status", "payment_status", "subtotal", "delivery_fee", "total", "created_at"],
  },
  deliveries: {
    label: "Entregas",
    table: "deliveries",
  ...
  cancellations: {
    ...
    statuses: ["canceled", "refunded"],
  },
  support: {
    label: "Suporte",
    table: "support_tickets",
    dateField: "created_at",
    cols: ["id", "order_id", "actor_role", "topic", "priority", "status", "created_at"],
  },
  documents: {
    label: "Documentos",
  ...
  users: {
    ...
    table: "profiles",
    dateField: "created_at",
    cols: ["id", "full_name", "role", "created_at"],
  },
  reviews: {
    label: "Avaliações",
    table: "order_reviews",
    dateField: "created_at",
    cols: ["id", "order_id", "author_role", "target_role", "rating", "comment", "visible", "created_at"],
  },
  enforcements: {
    label: "Suspensões e bloqueios",
    table: "account_enforcements",
    dateField: "created_at",
    cols: ["id", "profile_id", "action_type", "status", "reason", "starts_at", "ends_at", "created_at"],
  },
};
...
function cell(k, v) {
  ...
  if (["status", "document_status", "payment_status"].includes(k))
  ...
  if (typeof v === "object")
    return '<span class="cell-muted">' + esc(JSON.stringify(v).slice(0, 100)) + "</span>";
  const s = String(v);
  return s.length > 80 ? esc(s.slice(0, 77)) + "…" : esc(s);
}
...
function configure(c) {
  state.supabase = createClient(c.url, c.anonKey, {
...
async function boot() {
  ...
  if (!c?.url || !c?.anonKey) {
    ...
    if (!isLocalAdmin) {
      ...
      $("#setupView .auth-card p").innerHTML =
        "Configuração de ambiente ausente. Publique <code>admin/config.json</code> com apenas a URL do projeto e a chave publishable/anon. A produção não permite trocar o banco pelo navegador.";
    }
    return;
  }
  configure(c);
...
async function ensureAdminMfa() {
  ...
  if (factors.error) {
    loginError(factors.error.message);
    return "denied";
  }
  ...
  if (enrollment.error) {
    loginError(enrollment.error.message);
    return "denied";
  }
...
async function acceptSession(session) {
  ...
  const access = await state.supabase
    .from("admin_access")
    .select("active,is_superadmin")
  ...
  const permissions = await state.supabase
  ...
  $("#adminIdentity").textContent =
    (r.data.full_name || session.user.email) + (state.isSuperadmin ? " · Superadmin" : "");
  show("appView");
  ...
  if (firstAllowed) await openModule(firstAllowed[0]);
    ...
    $("#pageContent").innerHTML =
      '<div class="notice"><b>Seu acesso administrativo está ativo, mas nenhuma área foi liberada.</b><br>Solicite a um superadmin as permissões necessárias.</div>';
  }
  return "ready";
}
...
async function auditAdminSessionStart() {
  ...
  try {
    ...
    console.warn("Não foi possível registrar o login administrativo.", e);
  }
}
...
async function auditAdminSessionEnd() {
  ...
  const key = "feirae:admin:session-audited:" + state.session.user.id;
  try {
    await invokeAdminAction("session_event", { event: "logout" });
  } catch (e) {
    console.warn("Não foi possível registrar o logout administrativo.", e);
  }
  ...
  $("#nav").innerHTML = navGroups
    .map((group) => {
      ...
      return (
        ...
        allowed
          .map(
            (it) => '<button class="nav-item" data-module="' + esc(it[0]) + '">' + esc(it[1]) + "</button>",
          )
          .join("") +
        "</div>"
      );
...
async function openModule(id) {
  if (!canModule(id)) {
    toast("Você não possui permissão para esta área.");
    return;
  }
  state.active = id;
  state.rows = [];
  ...
  if (id === "dashboard") {
    ...
    $("#breadcrumb").textContent = "Operação";
    await renderDashboard();
    return;
  }
  if (id === "reports") {
    $("#pageTitle").textContent = "Relatórios";
    $("#breadcrumb").textContent = "Análises";
    await renderReports();
    return;
  }
  if (id === "alerts") {
  ...
  if (id === "stalls") {
    $("#pageTitle").textContent = "Bancas / Boxes";
  ...
  if (id === "finance") {
    ...
    $("#breadcrumb").textContent = "Comercial e financeiro";
    await renderFinance();
    return;
  }
  if (id === "integration_health") {
    $("#pageTitle").textContent = "Saúde das integrações";
    $("#breadcrumb").textContent = "Sistema";
    await renderIntegrationHealth();
    return;
  }
  if (id === "admins") {
    $("#pageTitle").textContent = "Administradores";
    $("#breadcrumb").textContent = "Sistema";
    await renderAdmins();
    return;
  }
  if (id === "audit") {
    $("#pageTitle").textContent = "Auditoria";
    $("#breadcrumb").textContent = "Sistema";
    await renderAuditAdvanced();
    return;
  }
  if (id === "settings") {
...
async function renderDashboard() {
  ...
  const ints =
    (
      await state.supabase
        ...
        .select("key,label,enabled,status,environment")
        .order("label")
    ).data || [];
  const cards = [
    ["Pedidos", vals[0]],
    ["Feirantes pendentes", vals[1]],
    ["Entregadores pendentes", vals[2]],
    ["Suportes abertos", vals[3]],
    ["Documentos a revisar", vals[4]],
    ["Saques solicitados", vals[5]],
  ];
  let html =
    '<div class="hero"><div><h2>Centro de operação do Feiraê</h2><p>Configurações, cadastros, regras, aprovações, suporte e financeiro em um só lugar.</p></div><div class="hero-badge"><b>' +
    new Date().toLocaleDateString("pt-BR") +
    "</b><span>status operacional</span></div></div>";
  if (vals.some((v) => v === null))
    html += '<div class="notice">Algumas métricas não puderam ser lidas. Confirme a migration 0003.</div>';
  html +=
    '<div class="kpi-grid">' +
    cards
      .map(
        (c) => '<div class="kpi"><span>' + esc(c[0]) + "</span><strong>" + (c[1] ?? "—") + "</strong></div>",
      )
      .join("") +
    "</div>";
  html +=
    '<div class="two-col"><section class="panel"><div class="panel-head"><div><h2>Pedidos recentes</h2><p>Últimas movimentações.</p></div></div><div class="list">';
  html += recent.length
    ? recent
        .map(
          (o) =>
            ...
            "</b><br><small>" +
            date(o.created_at) +
            '</small></div><div><span class="badge ' +
            esc(o.status) +
            '">' +
            esc(o.status) +
            "</span><br><small>" +
  ...
  html +=
    '</div></section><section class="panel"><div class="panel-head"><div><h2>Integrações</h2><p>Status operacional.</p></div></div><div class="list">';
  html += ints.length
    ? ints
        .map(
          (i) =>
            ...
            (i.enabled ? "active" : "false") +
            '">' +
            esc(i.status) +
            "</span></div>",
        )
        .join("")
    : '<div class="empty">Aplique a migration da gestão.</div>';
...
async function renderReports() {
  ...
  $("#pageContent").innerHTML =
    ...
    Object.entries(reportDefs)
      .map(([k, v]) => '<option value="' + esc(k) + '">' + esc(v.label) + "</option>")
      .join("") +
    "</select></label>" +
    '<label>De<input id="reportFrom" type="date" value="' +
    inputDate(start) +
    '"></label>' +
    '<label>Até<input id="reportTo" type="date" value="' +
    inputDate(now) +
    '"></label>' +
...
async function loadReport() {
  ...
  if (new Date(to) < new Date(from)) {
    toast("A data final precisa ser igual ou posterior à inicial.");
    return;
  }
  ...
  try {
    if (type === "summary") {
      ...
      state.reportColumns = def.cols;
      state.reportName = type;
      $("#reportResult").innerHTML =
        '<div class="kpi-grid report-kpis">' +
        reportKpi("Registros", rows.length) +
        (type === "orders"
          ? reportKpi("Valor total", money(rows.reduce((s, r) => s + Number(r.total || 0), 0)))
          : "") +
        (type === "payments"
          ? reportKpi("Volume pago", money(rows.reduce((s, r) => s + Number(r.amount || 0), 0)))
          : "") +
        (type === "deliveries"
          ? reportKpi("Taxas de entrega", money(rows.reduce((s, r) => s + Number(r.fee || 0), 0)))
          : "") +
        (type === "payouts"
          ? reportKpi("Valor dos repasses", money(rows.reduce((s, r) => s + Number(r.amount || 0), 0)))
          : "") +
        (type === "reviews" && rows.length
          ? reportKpi(
              "Nota média",
    ...
    if (state.reportTruncated) {
      $("#reportResult").insertAdjacentHTML(
        "afterbegin",
        '<div class="notice"><b>Volume alto:</b> a visualização no navegador foi limitada a 20.000 registros. Para períodos maiores, gere a exportação/agregação no backend.</div>',
      );
    }
  } catch (e) {
    state.reportRows = [];
    ...
    state.reportName = "";
    $("#reportResult").innerHTML =
      '<div class="notice"><b>Não foi possível gerar o relatório.</b><br>' +
...
function reportTableHtml(titleText, cols, rows) {
  return (
    '<section class="panel report-output"><div class="panel-head"><div><h2>' +
    esc(titleText) +
    "</h2><p>" +
    rows.length +
    " registros no período.</p></div></div>" +
    '<div class="table-wrap"><table><thead><tr>' +
    cols.map((k) => "<th>" + esc(label(k)) + "</th>").join("") +
    "</tr></thead><tbody>" +
    (rows.length
      ? rows
          .map((row) => "<tr>" + cols.map((k) => "<td>" + cell(k, row[k]) + "</td>").join("") + "</tr>")
          .join("")
      : '<tr><td colspan="' + cols.length + '" class="empty">Nenhum registro encontrado.</td></tr>') +
...
function exportCurrentReport() {
  ...
  const csv = [
    cols.map((k) => csvValue(label(k))).join(";"),
    ...rows.map((row) => cols.map((k) => csvValue(row[k])).join(";")),
  ].join("\n");
  const blob = new Blob(["\ufeff" + csv], { type: "text/csv;charset=utf-8" });
  const url = URL.createObjectURL(blob),
    a = document.createElement("a");
  a.href = url;
  a.download = "feirae-" + (state.reportName || "relatorio") + "-" + inputDate(new Date()) + ".csv";
    ...
    pairs
      .map(
        ([k, v]) =>
          ...
          esc(k) +
          "</span><strong>" +
          cell(k.toLowerCase().replaceAll(" ", "_"), v) +
          "</strong></div>",
      )
      .join("") +
    "</div>"
...
function miniTable(titleText, cols, rows) {
  return (
    '<section class="panel detail-section"><div class="panel-head"><div><h2>' +
    esc(titleText) +
    "</h2><p>" +
    rows.length +
    ' registro(s)</p></div></div><div class="table-wrap"><table><thead><tr>' +
    cols.map((k) => "<th>" + esc(label(k)) + "</th>").join("") +
    "</tr></thead><tbody>" +
    (rows.length
      ? rows
          .map((r) => "<tr>" + cols.map((k) => "<td>" + cell(k, r[k]) + "</td>").join("") + "</tr>")
          .join("")
      : '<tr><td colspan="' + cols.length + '" class="empty">Sem registros.</td></tr>') +
    "</tbody></table></div></section>"
  );
}
...
async function renderOrderDetail(orderId) {
  ...
  try {
    ...
    if (order.address_id)
      queries.push(state.supabase.from("addresses").select("*").eq("id", order.address_id).maybeSingle());
    const res = await Promise.all(queries);
    const bad = res.find((x) => x.error);
    if (bad) throw bad.error;
    const [items, vendors, delivery, payments, events, support, reviews, customer, address] = res;
    let html =
      '<div class="detail-head"><button class="secondary" id="backOrders">← Pedidos</button><div><b>#' +
      esc(order.id) +
      '</b><span class="badge ' +
      esc(order.status) +
      '">' +
      ...
      ((ORDER_TRANSITIONS[order.status] || []).length
        ? '<button class="primary" id="transitionOrder">Alterar etapa</button>'
        : "") +
      "</div></div>";
    html += detailPairs([
    ...
    if (address?.data)
      html += miniTable(
        "Endereço",
        ["recipient_name", "address_line", "number", "neighborhood", "city", "state", "postal_code"],
        [address.data],
      );
    html += miniTable(
      "Itens",
    ...
    html += miniTable(
      "Linha do tempo",
      ["created_at", "actor_role", "event_key", "label", "reason", "details"],
      events.data || [],
    );
    html += miniTable(
      "Suporte",
      ["created_at", "actor_role", "topic", "priority", "status", "details"],
      support.data || [],
    );
    html += miniTable(
      "Avaliações",
      ["created_at", "author_role", "target_role", "rating", "comment", "visible"],
      reviews.data || [],
    );
    $("#pageContent").innerHTML = html;
...
async function renderDeliveryDetail(deliveryId) {
  ...
  try {
    ...
    const [order, driver, vehicle, events, reviews] = await Promise.all([
      ...
      d.delivery_id
        ? state.supabase.from("profiles").select("id,full_name,phone").eq("id", d.delivery_id).maybeSingle()
        : Promise.resolve({ data: null, error: null }),
      d.vehicle_id
        ? state.supabase.from("delivery_vehicles").select("*").eq("id", d.vehicle_id).maybeSingle()
        : Promise.resolve({ data: null, error: null }),
      state.supabase
        .from("order_events")
        .select("*")
      ...
      state.supabase
        ...
        .select("*")
        .eq("order_id", d.order_id)
        .eq("target_role", "delivery")
        .order("created_at", { ascending: false }),
    ]);
    const bad = [order, driver, vehicle, events, reviews].find((x) => x.error);
    if (bad) throw bad.error;
    const canIntervene = !["collected", "out_for_delivery", "delivered", "canceled"].includes(
      String(d.status),
    );
    let html =
      '<div class="detail-head"><button class="secondary" id="backDeliveries">← Entregas</button><div><b>#' +
      esc(d.id) +
      '</b><span class="badge ' +
      esc(d.status) +
      '">' +
      ...
      "</span>" +
      (canIntervene
        ? '<button class="secondary" id="removeDeliveryDriver">Remover entregador</button><button class="secondary" id="reassignDelivery">Reatribuir</button><button class="danger-btn" id="cancelDelivery">Cancelar corrida</button>'
        : "") +
      "</div></div>";
    html += detailPairs([
      ["Pedido", d.order_id],
    ...
    if (vehicle.data)
      html += miniTable(
        "Veículo usado",
        ["vehicle_type", "brand_model", "plate", "capacity_kg", "active", "document_status"],
        [vehicle.data],
      );
    if (order.data)
      html += miniTable(
    ...
    html += miniTable(
      "Avaliações da entrega",
      ["created_at", "author_role", "rating", "comment", "visible"],
      reviews.data || [],
    );
    $("#pageContent").innerHTML = html;
    $("#backDeliveries").onclick = () => openModule("delivery_jobs");
    if ($("#removeDeliveryDriver"))
      $("#removeDeliveryDriver").onclick = () => runDeliveryAction("delivery_remove_driver", d);
...
function openOrderTransition(order) {
  ...
  if (!options.length) {
    toast("Este pedido não possui transição administrativa disponível.");
    return;
  }
  state.editing = { special: "order_transition", order };
  $("#modalTitle").textContent = "Alterar etapa do pedido";
  $("#editorForm").innerHTML =
    '<label>Status atual<input value="' +
    esc(order.status) +
    '" disabled></label>' +
    '<label>Próxima etapa<select id="secureNextStatus">' +
...
async function openDeliveryReassign(delivery) {
  ...
  if (profiles.error || approved.error) {
    toast(profiles.error?.message || approved.error?.message);
    return;
  }
  ...
  $("#editorForm").innerHTML =
    '<label class="full-span">Entregador aprovado<select id="secureDeliveryDriver"><option value="">Selecione</option>' +
...
function openDeliveryCancel(delivery) {
  ...
  $("#editorForm").innerHTML =
    '<label class="full-span">Motivo<textarea id="secureDeliveryReason" rows="4" required></textarea></label>';
  $("#modal").classList.remove("hidden");
}
...
function openDocumentReview(row) {
  ...
  $("#editorForm").innerHTML =
    '<label>Status<select id="secureDocumentStatus">' +
    ["under_review", "approved", "correction_required", "rejected"]
      .map((x) => '<option value="' + x + '" ' + (row.status === x ? "selected" : "") + ">" + x + "</option>")
      .join("") +
    "</select></label>" +
    '<label>Validade<input id="secureDocumentExpiry" type="date" value="' +
    esc(row.expires_at || "") +
    '"></label>' +
    '<label class="full-span">Motivo / correção<textarea id="secureDocumentReason" rows="4">' +
    esc(row.correction_reason || "") +
    "</textarea></label>";
  $("#modal").classList.remove("hidden");
}
...
function openSupportUpdate(row) {
  ...
  $("#editorForm").innerHTML =
    '<label>Prioridade<select id="secureSupportPriority"><option value="normal" ' +
    (row.priority === "normal" ? "selected" : "") +
    '>Normal</option><option value="urgent" ' +
    (row.priority === "urgent" ? "selected" : "") +
    ">Urgente</option></select></label>" +
    '<label>Status<select id="secureSupportStatus">' +
    ["open", "resolved", "closed"]
      .map((x) => '<option value="' + x + '" ' + (row.status === x ? "selected" : "") + ">" + x + "</option>")
      .join("") +
    "</select></label>" +
    '<label class="full-span">Detalhes<textarea id="secureSupportDetails" rows="5">' +
    esc(row.details || "") +
    "</textarea></label>";
  $("#modal").classList.remove("hidden");
}
...
function openReviewModeration(row) {
  ...
  $("#editorForm").innerHTML =
    '<label class="check-row"><input id="secureReviewVisible" type="checkbox" ' +
    (row.visible !== false ? "checked" : "") +
    "><span>Visível</span></label>" +
    '<label class="full-span">Motivo da moderação<textarea id="secureReviewReason" rows="4">' +
    esc(row.moderation_reason || "") +
    "</textarea></label>";
  $("#modal").classList.remove("hidden");
}
...
function openPrivacyUpdate(row) {
  ...
  $("#editorForm").innerHTML =
    '<label>Status<select id="securePrivacyStatus">' +
    ["open", "in_review", "completed", "rejected"]
      .map((x) => '<option value="' + x + '" ' + (row.status === x ? "selected" : "") + ">" + x + "</option>")
      .join("") +
    "</select></label>" +
    '<label class="full-span">Resolução<textarea id="securePrivacyNotes" rows="5">' +
    esc(row.resolution_notes || "") +
    "</textarea></label>";
  $("#modal").classList.remove("hidden");
}
...
async function openEnforcementCreate(profileId = null, actionType = "suspension") {
  ...
  const fixed = profileId
    ? '<label class="full-span">Usuário<input id="secureEnforcementProfile" value="' +
      esc(profileId) +
      '" readonly></label>'
  ...
  $("#editorForm").innerHTML =
    fixed +
    '<label>Ação<select id="secureEnforcementType">' +
    ["suspension", "ban", "orders_block", "sales_block", "deliveries_block"]
      .map((x) => '<option value="' + x + '" ' + (actionType === x ? "selected" : "") + ">" + x + "</option>")
      .join("") +
    "</select></label>" +
  ...
  if (!profileId) {
    ...
    $("#secureProfileSearch").oninput = (e) => {
      ...
      timer = setTimeout(async () => {
        ...
        $("#secureEnforcementProfile").innerHTML =
...
function openEnforcement(profileId, actionType) {
  if (!canModule("enforcements")) {
    toast("Sem permissão para suspensões e bloqueios.");
    return;
  }
...
async function renderProfile360(kind, row) {
  ...
  $("#pageTitle").textContent =
    kind === "vendor" ? "Feirante 360°" : kind === "delivery" ? "Entregador 360°" : "Cliente 360°";
  $("#breadcrumb").textContent = "Cadastros";
  $("#pageContent").innerHTML = '<div class="empty">Carregando visão completa…</div>';
  try {
    const profile = await state.supabase
      .from("profiles")
    ...
    if (kind === "customer") {
      ...
      if (canModule("support"))
        sections.push([
          "Suporte",
          await state.supabase
            .from("support_tickets")
            .select("id,topic,priority,status,created_at")
            .eq("opened_by", profileId)
            .order("created_at", { ascending: false })
            .limit(50),
          ["id", "topic", "priority", "status", "created_at"],
        ]);
      if (canModule("reviews"))
        sections.push([
          "Avaliações",
          await state.supabase
            .from("order_reviews")
            .select("id,author_role,target_role,rating,comment,created_at")
            .eq("author_id", profileId)
            .order("created_at", { ascending: false })
            .limit(50),
          ["id", "author_role", "target_role", "rating", "comment", "created_at"],
        ]);
    }
    if (kind === "vendor") {
      sections.push([
        "Feiras e boxes",
        await state.supabase
          .from("fair_vendor_memberships")
          .select("fair_id,stall_code,stall_name,active,updated_at")
          .eq("vendor_id", profileId),
        ["fair_id", "stall_code", "stall_name", "active", "updated_at"],
      ]);
      sections.push([
        "Lojas",
        await state.supabase
          .from("vendor_stores")
          .select("id,name,is_open,delivery_enabled,pickup_enabled,updated_at")
          .eq("vendor_id", profileId),
        ["id", "name", "is_open", "delivery_enabled", "pickup_enabled", "updated_at"],
      ]);
      sections.push([
        "Produtos",
        await state.supabase
          .from("products")
          .select("id,name,price,stock,available,updated_at")
          .eq("vendor_id", profileId)
          .order("updated_at", { ascending: false })
          .limit(100),
        ["id", "name", "price", "stock", "available", "updated_at"],
      ]);
    }
    if (kind === "delivery") {
      sections.push([
        ...
        await state.supabase
          ...
          .select("id,vehicle_type,brand_model,plate,capacity_kg,active,document_status,updated_at")
      ...
      sections.push([
        ...
        await state.supabase
          ...
          .select(
            "online,radius_km,preferred_distance_km,regions,auto_schedule,schedule_start,schedule_end,updated_at",
          )
          .eq("delivery_id", profileId),
        [
          "online",
      ...
      if (canModule("delivery_jobs"))
        sections.push([
          "Corridas",
          await state.supabase
            .from("deliveries")
            .select("id,order_id,status,fee,total_distance_km,accepted_at,delivered_at")
            .eq("delivery_id", profileId)
            .order("updated_at", { ascending: false })
            .limit(50),
          ["id", "order_id", "status", "fee", "total_distance_km", "accepted_at", "delivered_at"],
        ]);
    }
    if (canModule("documents"))
      sections.push([
        ...
        await state.supabase
          ...
          .select("id,document_type,status,expires_at,reviewed_at")
          .eq("profile_id", profileId)
          .order("updated_at", { ascending: false }),
        ["id", "document_type", "status", "expires_at", "reviewed_at"],
      ]);
    if (canModule("payouts"))
      sections.push([
        "Repasses",
        await state.supabase
          .from("payouts")
          .select("id,role,amount,status,requested_at,paid_at,created_at")
          .eq("profile_id", profileId)
          .order("created_at", { ascending: false })
          .limit(50),
        ["id", "role", "amount", "status", "requested_at", "paid_at", "created_at"],
      ]);
    if (canModule("enforcements"))
      sections.push([
        "Restrições",
        await state.supabase
          ...
          .eq("profile_id", profileId)
          .order("created_at", { ascending: false }),
        ["id", "action_type", "status", "reason", "starts_at", "ends_at", "created_at"],
      ]);

    const bad = sections.map((x) => x[1]).find((x) => x.error);
    if (bad) throw bad.error;
    let html =
      '<div class="detail-head"><button class="secondary" id="backProfile360">← Voltar</button><div><b>' +
      esc(profile.data?.full_name || profileId) +
      '</b><span class="badge active">' +
      esc(profile.data?.role || kind) +
      "</span></div></div>";
    html += detailPairs([
      ["Nome", profile.data?.full_name || "—"],
      ["Telefone", profile.data?.phone || "—"],
      ["Papel", profile.data?.role || kind],
      ["Cadastro", date(profile.data?.created_at)],
    ]);
    for (const [titleText, result, cols] of sections) html += miniTable(titleText, cols, result.data || []);
    $("#pageContent").innerHTML = html;
    $("#backProfile360").onclick = () =>
      openModule(kind === "vendor" ? "vendors" : kind === "delivery" ? "drivers" : "users");
  } catch (e) {
    renderError(e);
  }
}
...
async function renderStalls() {
  ...
  try {
    ...
    const bad = [memberships, fairs, vendors].find((x) => x.error);
    if (bad) throw bad.error;
    ...
    html += (memberships.data || []).length
      ? (memberships.data || [])
          .map((row, i) => {
            const fair = fairMap[row.fair_id],
              vendor = vendorMap[row.vendor_id];
            return (
              "<tr><td>" +
              esc(fair?.name || row.fair_id) +
              "</td><td>" +
              esc(vendor?.business_name || row.vendor_id) +
              "</td><td>" +
              esc(row.stall_code || "—") +
              "</td><td>" +
              esc(row.stall_name || "—") +
              "</td><td>" +
              cell("active", row.active) +
              "</td><td>" +
              date(row.updated_at) +
              '</td><td><div class="actions"><button data-stall-edit="' +
              i +
              '">Editar</button><button data-stall-toggle="' +
              i +
              '">' +
          ...
          .join("")
      : '<tr><td colspan="7" class="empty">Nenhuma banca/box cadastrada.</td></tr>';
    html += "</tbody></table></div></section>";
    $("#pageContent").innerHTML = html;
    $("#newStall").onclick = () => openStallEditor(null);
    document
      .querySelectorAll("[data-stall-edit]")
      .forEach((b) => (b.onclick = () => openStallEditor(memberships.data[Number(b.dataset.stallEdit)])));
    document.querySelectorAll("[data-stall-toggle]").forEach(
      (b) =>
        (b.onclick = async () => {
          ...
          toast(row.active ? "Banca desativada." : "Banca ativada.");
          await renderStalls();
        }),
    );
  } catch (e) {
...
function openStallEditor(row) {
  ...
  $("#editorForm").innerHTML =
    '<label>Feira<select id="stallFair"><option value="">Selecione</option>' +
    ctx.fairs
      .map(
        (x) =>
          '<option value="' +
          esc(x.id) +
          '" ' +
          (row?.fair_id === x.id ? "selected" : "") +
          ">" +
          esc(x.name + " · " + (x.city || x.state || "")) +
          "</option>",
      )
      .join("") +
    "</select></label>" +
    '<label>Feirante<select id="stallVendor"><option value="">Selecione</option>' +
    ctx.vendors
      .map(
        (x) =>
          '<option value="' +
          esc(x.id) +
          '" ' +
          (row?.vendor_id === x.id ? "selected" : "") +
          ">" +
          esc(x.business_name + (x.approved ? "" : " · pendente")) +
          "</option>",
      )
      .join("") +
    "</select></label>" +
    '<label>Código do box<input id="stallCode" value="' +
    esc(row?.stall_code || "") +
    '"></label>' +
    '<label>Nome da banca/box<input id="stallName" value="' +
    esc(row?.stall_name || "") +
    '"></label>' +
    '<label class="full-span">Horários próprios (JSON)<textarea id="stallHours" rows="4">' +
    esc(row?.custom_opening_hours ? JSON.stringify(row.custom_opening_hours, null, 2) : "{}") +
    "</textarea></label>" +
    '<label class="full-span">Observações<textarea id="stallNotes" rows="3">' +
    esc(row?.notes || "") +
    "</textarea></label>" +
    '<label class="check-row"><input id="stallActive" type="checkbox" ' +
    (row?.active !== false ? "checked" : "") +
    "><span>Ativa</span></label>";
  $("#modal").classList.remove("hidden");
}

async function saveStallEditor() {
  const { row, isNew } = state.editing;
  const fair_id = $("#stallFair").value,
    vendor_id = $("#stallVendor").value;
  if (!fair_id || !vendor_id) {
    toast("Selecione a feira e o feirante.");
    return false;
  }
  let hours;
  try {
    hours = JSON.parse($("#stallHours").value || "{}");
  } catch {
    toast("Horários em JSON inválido.");
    return false;
  }
  const payload = {
    fair_id,
    vendor_id,
    stall_code: $("#stallCode").value.trim() || null,
    stall_name: $("#stallName").value.trim() || null,
    custom_opening_hours: hours,
    notes: $("#stallNotes").value.trim() || null,
    active: $("#stallActive").checked,
...
async function renderFinance() {
  ...
  try {
    ...
    const expected = rows.reduce((s, r) => {
      const allocated =
        Number(r.provider_fee || 0) +
        Number(r.platform_amount || r.commission || 0) +
        Number(r.vendor_amount || 0) +
        Number(r.delivery_amount || 0) +
    ...
    let html =
      '<div class="kpi-grid">' +
      reportKpi("Recebido bruto", money(gross)) +
      reportKpi("Taxas do provedor", money(provider)) +
      reportKpi("Receita Feiraê", money(platform)) +
      reportKpi("Feirantes", money(vendors)) +
      reportKpi("Entregadores", money(drivers)) +
      reportKpi("Reembolsado", money(refunded)) +
      reportKpi("Diferença a conciliar", money(expected)) +
      reportKpi("Repasses pagos", money(paidPayouts)) +
      "</div>";
    html += miniTable(
    ...
    html +=
      '<div class="notice"><b>Segurança financeira:</b> confirmar cobrança, estornar no provedor e liquidar repasses continuam sendo operações server-side. A Gestão consulta e marca conciliação; não usa chave secreta no navegador.</div>';
    $("#pageContent").innerHTML = html;
  } catch (e) {
    renderError(e);
  }
  ...
  try {
    ...
    html += rows.length
      ? rows
          ...
          .join("")
      : '<tr><td colspan="7" class="empty">Nenhum alerta ativo.</td></tr>';
    html += "</tbody></table></div></section>";
    $("#pageContent").innerHTML = html;
    $("#refreshAlerts").onclick = () => renderAlerts();
    document
      .querySelectorAll("[data-alert-ack]")
      .forEach((b) => (b.onclick = () => updateAlert("alert_acknowledge", rows[Number(b.dataset.alertAck)])));
    document
      .querySelectorAll("[data-alert-resolve]")
      .forEach((b) => (b.onclick = () => updateAlert("alert_resolve", rows[Number(b.dataset.alertResolve)])));
  } catch (e) {
    renderError(e);
  }
}
...
async function renderIntegrationHealth() {
  ...
  try {
    ...
    for (const e of events.data || []) {
      if (!latest[e.integration_key]) latest[e.integration_key] = e;
    }
    const rows = (registry.data || []).map((r) => ({
      ...r,
      health_status: latest[r.key]?.status || r.status,
      health_message: latest[r.key]?.message || "",
      response_ms: latest[r.key]?.response_ms,
      checked_at: latest[r.key]?.checked_at || r.last_checked_at,
    }));
    let html =
      '<div class="notice"><b>Health check seguro:</b> o navegador solicita a verificação, mas a URL/segredo fica no ambiente da Edge Function.</div>' +
      ...
      rows
        .map(
          (row) =>
            ...
            esc(row.environment) +
            "</td><td>" +
            cell("status", row.health_status) +
            "</td><td>" +
            esc(row.response_ms ?? "—") +
            " ms</td><td>" +
            date(row.checked_at) +
            '</td><td><button class="secondary" data-health-check="' +
            esc(row.key) +
            '">Testar</button></td></tr>',
        )
        .join("") +
    ...
    html += miniTable(
      ...
      ["integration_key", "status", "message", "response_ms", "checked_at"],
      events.data || [],
    );
    $("#pageContent").innerHTML = html;
    document
      .querySelectorAll("[data-health-check]")
      .forEach((b) => (b.onclick = () => runHealthCheck(b.dataset.healthCheck)));
  } catch (e) {
    renderError(e);
  }
}
...
async function renderAdmins() {
  ...
  try {
    ...
    const permMap = (permissions.data || []).reduce((a, p) => {
      (a[p.profile_id] ??= []).push(p.permission);
      return a;
    }, {});
    let html =
      '<section class="panel"><div class="panel-head"><div><h2>Administradores</h2><p>Ative/desative acessos e configure permissões sem compartilhar acesso total.</p></div></div>' +
      ...
      (candidates.data || [])
        .map(
          (u) =>
            ...
            esc(u.id) +
            '">' +
            esc((u.full_name || "Sem nome") + " · " + u.role) +
            "</option>",
        )
        .join("") +
      '</select></label><button class="primary" id="promoteAdmin">Promover selecionado</button></div><div class="admin-list">';
    for (const p of profiles.data || []) {
      const ac = accessMap[p.id],
        active = ac?.active === true,
        isSuper = ac?.is_superadmin === true,
        perms = permMap[p.id] || [];
      html +=
        '<div class="admin-card"><div><strong>' +
        esc(p.full_name || p.id) +
        "</strong><small>" +
        esc(p.id) +
        '</small><div class="permission-chips">' +
        (isSuper
          ? "<span>SUPERADMIN</span>"
          : perms.length
            ? perms.map((x) => "<span>" + esc(x) + "</span>").join("")
            : "<span>SEM PERMISSÕES</span>") +
        '</div></div><div class="actions"><span class="badge ' +
        (active ? "true" : "false") +
        '">' +
        (active ? "Ativo" : "Desativado") +
        "</span>" +
        (isSuper ? '<span class="badge active">Superadmin</span>' : "") +
        '<button data-admin-access="' +
        esc(p.id) +
        '" data-active="' +
        active +
        '">' +
        ...
        "</button>" +
        (state.isSuperadmin
          ? '<button data-admin-super="' +
            esc(p.id) +
            '" data-super="' +
            isSuper +
            '">' +
            (isSuper ? "Remover superadmin" : "Tornar superadmin") +
            "</button>"
          : "") +
        '<button data-admin-permissions="' +
        esc(p.id) +
        '">Permissões</button></div></div>';
    }
    html +=
      '</div></section><div class="notice">Promover alguém a administrador altera um papel sensível e exige a permissão <code>permissions.manage</code>. O painel não cria contas do Auth nem envia convite usando service role.</div>';
    $("#pageContent").innerHTML = html;
    $("#promoteAdmin").onclick = promoteExistingAdmin;
    document
      .querySelectorAll("[data-admin-access]")
      .forEach((b) => (b.onclick = () => setAdminAccess(b.dataset.adminAccess, b.dataset.active !== "true")));
    document
      .querySelectorAll("[data-admin-super]")
      .forEach(
        (b) => (b.onclick = () => setAdminSuperadmin(b.dataset.adminSuper, b.dataset.super !== "true")),
      );
    document
      .querySelectorAll("[data-admin-permissions]")
      .forEach(
        (b) =>
          (b.onclick = () =>
            editAdminPermissions(b.dataset.adminPermissions, permMap[b.dataset.adminPermissions] || [])),
      );
  } catch (e) {
    renderError(e);
  }
...
async function setAdminSuperadmin(profileId, isSuperadmin) {
  if (!state.isSuperadmin) {
    toast("Somente superadmin pode alterar este nível.");
    return;
  }
  if (!confirm(isSuperadmin ? "Conceder acesso irrestrito de superadmin?" : "Remover acesso de superadmin?"))
    return;
  try {
...
function editAdminPermissions(profileId, current) {
  ...
  $("#editorForm").innerHTML =
    '<div class="full-span permission-editor"><p>Marque as áreas permitidas. Sem permissões explícitas, o administrador comum não acessa módulos protegidos.</p>' +
    ADMIN_PERMISSION_SET.map(
      (p) =>
        '<label class="check-row"><input type="checkbox" data-admin-perm="' +
        esc(p) +
        '" ' +
        (current.includes(p) ? "checked" : "") +
        "><span>" +
        esc(p) +
        "</span></label>",
    ).join("") +
...
async function saveAdminPermissions() {
  ...
  const selected = [...$("#editorForm").querySelectorAll("[data-admin-perm]:checked")].map(
    (x) => x.dataset.adminPerm,
  );
  try {
...
async function renderAuditAdvanced() {
  const admins = await state.supabase
    .from("profiles")
    .select("id,full_name")
    .eq("role", "admin")
    .order("full_name");
  const adminOptions = (admins.data || [])
    .map((x) => '<option value="' + esc(x.id) + '">' + esc(x.full_name || x.id) + "</option>")
    .join("");
  $("#pageContent").innerHTML =
    ...
    '<label>Administrador<select id="auditAdmin"><option value="">Todos</option>' +
    adminOptions +
    '</select></label><label>Entidade<input id="auditEntity" placeholder="orders, fairs…"></label>' +
...
async function loadAuditAdvanced() {
  let q = state.supabase
    ...
    .order("created_at", { ascending: false })
    .limit(1000);
  const admin = $("#auditAdmin")?.value.trim(),
    entity = $("#auditEntity")?.value.trim(),
    action = $("#auditAction")?.value.trim(),
    from = $("#auditFrom")?.value,
    to = $("#auditTo")?.value;
  if (admin) q = q.eq("admin_id", admin);
  ...
  const r = await q;
  if (r.error) {
    $("#auditResult").innerHTML = '<div class="notice">' + esc(r.error.message) + "</div>";
    return;
  }
  $("#auditResult").innerHTML = miniTable(
    "Eventos de auditoria",
    ["created_at", "admin_id", "action", "entity", "entity_id", "before_data", "after_data"],
    r.data || [],
  );
}
...
async function renderSettings() {
  ...
  const grouped = (r.data || []).reduce((a, row) => {
    (a[row.category] ??= []).push(row);
    return a;
  }, {});
  let html =
    '<div class="notice"><b>Configuração sem código:</b> não use esta área para senhas, tokens ou chaves secretas.</div>';
  for (const [cat, rows] of Object.entries(grouped)) {
    html +=
      ...
      "</h2><p>" +
      rows.length +
      ' parâmetros</p></div></div><div class="config-grid" style="padding:16px">';
    html += rows.map(settingCard).join("");
    html += "</div></section>";
  }
  $("#pageContent").innerHTML = html;
  document
    .querySelectorAll("[data-save-setting]")
    .forEach((b) => (b.onclick = () => saveSetting(b.dataset.saveSetting)));
}

function settingCard(r) {
  const v = r.value,
    bool = typeof v === "boolean",
    primitive = ["string", "number", "boolean"].includes(typeof v);
  let input;
  if (bool)
    input =
      '<select data-setting-value="' +
      esc(r.key) +
      '"><option value="true" ' +
      (v ? "selected" : "") +
      '>Ativado</option><option value="false" ' +
      (!v ? "selected" : "") +
      ">Desativado</option></select>";
  else
    input =
      '<input data-setting-value="' +
      esc(r.key) +
      '" value="' +
      esc(primitive ? v : JSON.stringify(v)) +
      '">';
  return (
    '<div class="config-card"><h3>' +
    esc(r.label) +
    "</h3><p>" +
    esc(r.description || r.key) +
    '</p><div class="value-row">' +
    input +
    '<button class="secondary" data-save-setting="' +
    esc(r.key) +
...
async function saveSetting(key) {
  ...
  const r = await state.supabase
    ...
    .update({ value: v, updated_by: state.session.user.id, updated_at: new Date().toISOString() })
    .eq("key", key);
...
async function renderModule(m) {
  ...
  if (!state.rows.length && state.page > 0) {
    state.page--;
...
function drawTable(m, rows) {
  ...
  let html =
    ...
    "</h2><p>" +
    esc(m.desc) +
    '</p></div><div class="toolbar"><input id="tableSearch" value="' +
    esc(state.query) +
    '" placeholder="Buscar no banco">';
  if (m.bulkField)
    html +=
      '<button id="bulkEnable" class="secondary">Ativar selecionados</button><button id="bulkDisable" class="secondary">Desativar selecionados</button>';
  if (m.create) html += '<button id="newRecord" class="primary">Novo</button>';
  html += '</div></div><div class="table-wrap"><table><thead><tr>';
  if (m.bulkField)
    html += '<th><input id="selectAllRows" type="checkbox" aria-label="Selecionar todos"></th>';
  ...
  html +=
    ...
    " de " +
    state.total +
    '</span><div><button id="prevPage" class="secondary" ' +
    (state.page === 0 ? "disabled" : "") +
    '>Anterior</button><button id="nextPage" class="secondary" ' +
    (end >= state.total ? "disabled" : "") +
    ">Próxima</button></div></div></section>";
  $("#pageContent").innerHTML = html;
  fillRows(m, rows);
  let searchTimer;
  ...
  if (m.create)
    $("#newRecord").onclick = () =>
      m.secureAction === "enforcement" ? openEnforcementCreate() : openEditor(m, null);
  if (m.bulkField) {
...
function fillRows(m, rows) {
  ...
  if (!rows.length) {
    body.innerHTML =
      '<tr><td colspan="' +
      (m.cols.length + (m.readonly ? 0 : 1) + extra) +
      '" class="empty">Nenhum registro encontrado.</td></tr>';
    return;
  }
  body.innerHTML = rows
    .map((row, i) => {
      ...
      if (m.bulkField)
        line +=
          '<td><input type="checkbox" data-select-row="' +
          i +
          '" ' +
      ...
      if (!m.readonly) {
        ...
        if (m.approvalKind)
          line +=
            '<button data-profile-approval="' +
            i +
            '">' +
            (row.approved ? "Revogar aprovação" : "Aprovar") +
            "</button>";
        if (m.secureAction === "document_review")
          line += '<button data-document-review="' + i + '">Revisar</button>';
        if (m.secureAction === "support_update")
          line += '<button data-support-update="' + i + '">Atender</button>';
        if (m.secureAction === "payment_reconcile")
          line +=
            '<button data-payment-reconcile="' +
            i +
            '">' +
            (row.reconciled ? "Desconciliar" : "Conciliar") +
            "</button>";
        if (m.secureAction === "review_moderate")
          line += '<button data-review-moderate="' + i + '">Moderar</button>';
        if (m.secureAction === "privacy_update")
          line += '<button data-privacy-update="' + i + '">Tratar</button>';
        if (m.secureAction === "enforcement" && row.status === "active")
          line += '<button data-enforcement-revoke="' + i + '">Revogar</button>';
        if (m.fields?.length) line += '<button data-edit="' + i + '">Editar</button>';
        if (m.documentViewer && row.file_path)
          line += '<button data-document="' + i + '">Abrir documento</button>';
        if (m.enforcementTarget && canModule("enforcements")) {
          line +=
            '<button data-suspend="' + i + '">Suspender</button><button data-ban="' + i + '">Banir</button>';
          if (m.table === "delivery_profiles")
            line += '<button data-block-delivery="' + i + '">Bloquear entregas</button>';
        }
    ...
    .join("");
  body.querySelectorAll("[data-select-row]").forEach(
    (b) =>
      (b.onchange = () => {
        const row = rows[Number(b.dataset.selectRow)],
          key = String(row[m.key || "id"]);
        if (b.checked) state.selected.add(key);
        else state.selected.delete(key);
      }),
  );
  body
    .querySelectorAll("[data-order-detail]")
    .forEach((b) => (b.onclick = () => renderOrderDetail(rows[Number(b.dataset.orderDetail)].id)));
  body
    .querySelectorAll("[data-delivery-detail]")
    .forEach((b) => (b.onclick = () => renderDeliveryDetail(rows[Number(b.dataset.deliveryDetail)].id)));
  body
    .querySelectorAll("[data-profile-detail]")
    .forEach(
      (b) => (b.onclick = () => renderProfile360(m.profileDetail, rows[Number(b.dataset.profileDetail)])),
    );
  body
    .querySelectorAll("[data-profile-approval]")
    .forEach((b) => (b.onclick = () => toggleProfileApproval(m, rows[Number(b.dataset.profileApproval)])));
  body
    .querySelectorAll("[data-document-review]")
    .forEach((b) => (b.onclick = () => openDocumentReview(rows[Number(b.dataset.documentReview)])));
  body
    .querySelectorAll("[data-support-update]")
    .forEach((b) => (b.onclick = () => openSupportUpdate(rows[Number(b.dataset.supportUpdate)])));
  body
    .querySelectorAll("[data-payment-reconcile]")
    .forEach((b) => (b.onclick = () => togglePaymentReconcile(rows[Number(b.dataset.paymentReconcile)])));
  body
    .querySelectorAll("[data-review-moderate]")
    .forEach((b) => (b.onclick = () => openReviewModeration(rows[Number(b.dataset.reviewModerate)])));
  body
    .querySelectorAll("[data-privacy-update]")
    .forEach((b) => (b.onclick = () => openPrivacyUpdate(rows[Number(b.dataset.privacyUpdate)])));
  body
    .querySelectorAll("[data-enforcement-revoke]")
    .forEach((b) => (b.onclick = () => revokeEnforcement(rows[Number(b.dataset.enforcementRevoke)])));
  body
    .querySelectorAll("[data-edit]")
    .forEach((b) => (b.onclick = () => openEditor(m, rows[Number(b.dataset.edit)])));
  body
    .querySelectorAll("[data-document]")
    .forEach((b) => (b.onclick = () => openDocument(rows[Number(b.dataset.document)])));
  body
    .querySelectorAll("[data-suspend]")
    .forEach((b) => (b.onclick = () => openEnforcement(rows[Number(b.dataset.suspend)].id, "suspension")));
  body
    .querySelectorAll("[data-ban]")
    .forEach((b) => (b.onclick = () => openEnforcement(rows[Number(b.dataset.ban)].id, "ban")));
  body
    .querySelectorAll("[data-block-delivery]")
    .forEach(
      (b) =>
        (b.onclick = () => openEnforcement(rows[Number(b.dataset.blockDelivery)].id, "deliveries_block")),
    );
  body
    .querySelectorAll("[data-delete]")
    .forEach((b) => (b.onclick = () => removeRow(m, rows[Number(b.dataset.delete)])));
}

async function bulkSet(m, value) {
  if (!m.bulkField || !state.selected.size) {
    toast("Selecione pelo menos um registro.");
    return;
  }
  const key = m.key || "id";
...
async function openDocument(row) {
  ...
  if (!path) {
    toast("Este cadastro não possui arquivo enviado.");
    return;
  }
  ...
  const setting = await state.supabase
    .from("platform_settings")
    .select("value")
    .eq("key", "documents.storage_bucket")
    .maybeSingle();
  ...
  if (needsState)
    jobs.push(
      state.supabase
        ...
        .then((r) => {
          if (!r.error)
            options.state = (r.data || []).map((x) => ({ value: x.code, label: x.code + " · " + x.name }));
        }),
    );
  if (needsRegion)
    jobs.push(
      state.supabase
        ...
        .then((r) => {
          if (!r.error)
            options.region_id = (r.data || []).map((x) => ({
              value: x.id,
      ...
      state.supabase
        ...
        .then((r) => {
          if (!r.error) {
            ...
            options.vehicle_code = rows;
            options.vehicle_type = rows;
          }
        }),
    );
...
function renderField(field, v) {
  ...
  if (dynamic) {
    return (
      ...
      '" data-field="' +
      field.key +
      '" data-type="select"><option value="">Selecione</option>' +
      dynamic
        .map(
          (o) =>
            '<option value="' +
  ...
  if (field.type === "checkbox")
    return (
      '<label class="check-row"><input id="' +
      id +
      '" data-field="' +
      field.key +
      '" type="checkbox" ' +
  ...
  if (field.type === "textarea" || field.type === "json") {
    const value =
      field.type === "json" && v && typeof v === "object" ? JSON.stringify(v, null, 2) : (v ?? "");
    return (
    ...
    field.type === "number"
      ...
      : field.type === "date"
        ...
        : field.type === "datetime"
          ...
          : "text";
  let value = v ?? "";
  if (field.type === "datetime" && v) value = new Date(v).toISOString().slice(0, 16);
  return (
    "<label>" +
    esc(field.label) +
    '<input id="' +
  ...
  $("#editorForm")
    ...
    .forEach((input) => {
      const k = input.dataset.field,
        t = input.dataset.type || (input.type === "checkbox" ? "checkbox" : "text");
...
async function saveEditor() {
  ...
  if (state.editing.special) {
    ...
    try {
      ...
      if (result) {
        closeEditor();
        toast(result.message || "Alteração salva.");
  ...
  const { m, row, isNew } = state.editing;
  let payload;
  try {
    payload = readPayload();
  } catch {
...
function renderError(e) {
  $("#pageContent").innerHTML =
    '<div class="notice"><b>Não foi possível carregar esta área.</b><br>' +
    esc(e.message || String(e)) +
    "<br><br>Confirme a migration <code>0003_management_console.sql</code> e o papel <code>admin</code>.</div>";
}

$("#setupForm").onsubmit = (e) => {
  e.preventDefault();
  if (!isLocalAdmin) {
    loginError("A conexão da Gestão é fixa em produção.");
    return;
  }
...
$("#loginForm").onsubmit = async (e) => {
  e.preventDefault();
  loginError("");
  const r = await state.supabase.auth.signInWithPassword({
    email: $("#email").value.trim(),
    password: $("#password").value,
  });
  if (r.error) {
    loginError(r.error.message);
    return;
  }
...
boot().catch((e) => {
  console.error(e);
  show("loginView");
  loginError("Falha ao iniciar. Verifique a conexão com o Supabase.");
});
